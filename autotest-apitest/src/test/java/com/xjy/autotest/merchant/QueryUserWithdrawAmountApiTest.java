 ...
 */
public class QueryUserWithdrawAmountApiTest extends OpenapiTestBasis{

	@AutoTest(file = "merchant/queryUserWithdrawAmountApiTestSuccess.csv")
	@DisplayName("查询提现余额--成功用例")
	public void queryUserWithdrawAmountApiTestSuccess(
			int testId,
			String status,
			String code,
			// 基本参数
		...
		//初始化数据
		service = "queryUserWithdrawAmount";
		partnerId = pId;
		securityCheckCode = secretKey;
        merchantOrderNo = "merchantOrderNo" + System.currentTimeMillis();
		userId = "S0301912311000092151";
		userId = "S0301912311000092149";
		//清除数据
		//准备数据
		//测试过程
        //主订单
		...
		dataMap.put("userId", userId);//用户唯一id
        //签名
		...
		//结果验证
    	print("查询提现余额response:" + response.toString());
