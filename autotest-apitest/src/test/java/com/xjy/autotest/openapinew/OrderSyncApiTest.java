 ...
 */
public class OrderSyncApiTest extends OpenapiTestBasis {

    @AutoTest(file = "openapinew/orderSyncApiTestSuccess.csv")
    @DisplayName("订单同步--成功用例")
    public void orderSyncApiTestSuccess(
            int testId,
            String status,
            String code,
            // 基本参数
        ...
        //merchantOrderNo = "15625734942158c6cd3a";

        bizNo = "1jeaoAt3t0s051hge8er";
        //清除数据
        //准备数据
        //测试过程
        //主订单
        ...
        //结果验证
        print("订单同步response:" + response.toString());
        JSONObject result = JSONObject.parseObject(response.getBody());
        assertEquals(status, result.getString("status"));
        assertEquals(code, result.getString("code"));
        JSONObject info = result.getJSONObject("info");
        assertEquals("SUCCESS", info.getString("tradeState"));
        //数据验证
        //清除数据
    }
}
