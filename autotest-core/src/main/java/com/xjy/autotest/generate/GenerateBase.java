...
public class GenerateBase {
    ...
    public void generateBase(String projectName, List<GeneratedJavaFile> generatedJavaFiles, boolean covered) {
        try {
            ...
            String parm = prop.getProperty("ds." + projectName + ".parms");
            Set<String> tabs = new HashSet();
            if (StringUtils.isNotBlank(parm)) {
                String[] parms = parm.split(",");
                tabs = new HashSet(Arrays.asList(parms));
            }
                    ...
                    if (fileName.equals(gjf.getCompilationUnit().getType().getShortNameWithoutTypeArguments())) {
                        ...
                        for (String p : tabs) {
                            if (p.contains(table)) {
                                ...
                                if (matcher.find()) {
                                    pls.add(matcher.group().substring(1, matcher.group().length() - 1).trim());
                                }
                            }
                        }
                        for (Field f : tlc.getFields()) {
                            ...
                            if(StringUtils.isBlank(clumnName)){
                                par.setComment(par.getName());
                            }else{
                                par.setComment(clumnName);
                            }
                            pars.add(par);
                        }
                        tableMap.put("pars", pars);
                        tableMap.put("pls", pls);
            ...
            String outFile = getClassFilePath(GenerateBase.class)
                    ...
                    .replace("generate/" + GenerateBase.class.getSimpleName() + ".class", "testbase/"
                            + className + ".java");
            String packageName = "com.xjy.autotest.testbase";
            ...
            String outFile3 = getClassFilePath(GenerateBase.class)
                    .replace("\\target\\classes", "\\src\\main\\java")
                    .replace("/target/classes", "/src/main/java")
                    .replace("generate\\" + GenerateBase.class.getSimpleName() + ".class", "config\\" + className3 + ".java")
                    .replace("generate/" + GenerateBase.class.getSimpleName() + ".class", "config/" + className3 + ".java");
