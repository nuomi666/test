...
/**
 * Created by ychaoyang on 2018/5/22.
 */
	...
	public void generate(Class<?> facadeCla, String methodName, String packag, String author, Class<?> claz) {
		...
		for (Method m : methods) {
			...
			for (Parameter p : parameters) {
				...
				orderName = toLowerCaseFirstOne(orderType.split("\\.")[orderType.split("\\.").length - 1]);
...
//			for (Param p : pars) {
//				if (p.getTypeName().contains("Enum") || p.getTypeName().contains("Money")
//						|| p.getTypeName().contains("List") || p.getTypeName().contains("Date") || p.getTypeName()
//						.contains("Map")) {
//					importlist.add(p.getType());
//					if (p.getTypeName().contains("Map")) {
...
//				addParameter(clz, par, list);
//						if (null == p.getFather()) {
//							p.setFather(f.getName());
//						} else {
//							p.setFather(p.getFather() + "_" + f.getName());
//						}
				//暂时不考虑第三层
//					list.add(p);
			}
