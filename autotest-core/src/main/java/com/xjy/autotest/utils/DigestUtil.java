...
//				throw new IllegalArgumentException(entry.getKey() + " 待签名值不能为空");
	 ...
	 * @return
	 */
	public static <T> String digest2(Map<String, T> dataMap, String securityCheckKey, DigestALGEnum de, String
			encoding) {
		if (dataMap == null) {
			throw new IllegalArgumentException("数据不能为空");
		}
...
//		sb.deleteCharAt(sb.length() - 1);
		...
		sb.append("key="+securityCheckKey);
