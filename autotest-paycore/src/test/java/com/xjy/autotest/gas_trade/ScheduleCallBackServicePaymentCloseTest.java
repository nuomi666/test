 ...
 * @date 2018/9/6
 */
public class ScheduleCallBackServicePaymentCloseTest extends SpringBootTestBase {
    @Reference(group = "gas_trade_payment_close", version = "1.0")
    ScheduleCallBackService paymentClose;
...
//    Gas_tradeTestBase gasTradeTestBase;
     ...
     * 1001
     */
    @AutoTest(file = "gas_trade/scheduleCallBackServicePaymentCloseTestSuccess.csv")
    @DisplayName("付款挂起定时重试--成功用例")
    public void scheduleCallBackServicePaymentCloseTestSuccess(
            // 基本参数
            int testId,
            Status status,
            String code,
...
//        assertEquals("G03_02_0001", paymentInfo.getCode());
        // 测试过程
        // 调用接口
        paymentClose.justDoIT();
        // 结果验证
        // 数据验证
//        GasTradePaymentDO paymentInfoxx = gasTradeTestBase.findGasTradePaymentByReqId(reqId).get(0);
//        assertEquals("closed", paymentInfoxx.getState());
//        assertEquals("fail", paymentInfoxx.getStatus());
//        assertEquals("G03_03_0003", paymentInfoxx.getCode());
        // 清除数据
    }
}
