 ...
 */
@DisplayName("信用还款")
public class AccountTransServiceCreditRePaymentTest extends SpringBootTestBase {
    ...
    /**
     * 1001 通过accountNo信用还款100元
     * 1002 通过userId+tag信用还款101元
     */
    @AutoTest(file = "user/accountTransServiceCreditRePaymentTestSuccess.csv")
    @DisplayName("成功用例")
    public void accountTransServiceCreditRePaymentTestSuccess(
            // 基本参数
            int testId,
            Status status,
            String code,
        ...
        accountDO.setBalance(accountDO.getBalance() + order.getAmount().getCent());
        ...
        // 调用接口
        BizResult result = accountTransService.creditRePayment(order);
        // 结果验证
        print(result);
     ...
     * 1004 账户传空
     */
    @AutoTest(file = "user/accountTransServiceCreditRePaymentTestFailOne.csv")
    @DisplayName("参数缺省")
    public void accountTransServiceCreditRePaymentTestFailOne(
            // 基本参数
            int testId,
            Status status,
            String code,
     ...
     * 1004 信用账户余额不足
     */
    @AutoTest(file = "user/accountTransServiceCreditRePaymentTestFailTwo.csv")
    @DisplayName("失败用例")
    public void accountTransServiceCreditRePaymentTestFailTwo(
            // 基本参数
            int testId,
            String testMemo,
            Status status,
        ...
        if (testId == 1004) {
            ...
            accountDO1.setCreditAmount(order.getAmount().getCent() - 2);
            accountDO1.setBalance(order.getAmount().getCent() - 2);
