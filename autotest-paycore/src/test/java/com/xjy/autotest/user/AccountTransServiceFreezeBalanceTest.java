 ...
 */
public class AccountTransServiceFreezeBalanceTest extends SpringBootTestBase {
    ...
    /**
     * 1001 必填参数，标准账户，通用冻结，已冻结金额为0，余额1000，冻结200
     * 1002 全填参数，标准账户，充值冻结，已冻结金额为100，余额1100，冻结1000
     * 1003 全填参数，标准账户，提现冻结，已冻结金额为200，余额1000，冻结100
     * 1004 全填参数，信用账户，通用冻结，已冻结金额为300，余额1300，冻结1000
     * 1005 全填参数，信用账户，充值冻结，已冻结金额为400，余额1000，冻结300
     * 1006 全填参数，信用账户，提现冻结，已冻结金额为500，余额1000，冻结100
     * 1007 全填参数，结算账户，通用冻结，已冻结金额为600，余额1000，冻结100
     * 1008 全填参数，结算账户，充值冻结，已冻结金额为700，余额2000，冻结1000
     * 1009 全填参数，结算账户，提现冻结，已冻结金额为800，余额2000，冻结400
     * 1010 全填参数，中间过度账户，通用冻结，已冻结金额为900，余额1500，冻结500
     * 1011 全填参数，中间过度账户，充值冻结，已冻结金额为1000，余额1500，冻结100
     * 1012 全填参数，中间过度账户，提现冻结，已冻结金额为1100，余额3000，冻结1000
     * 1013 全填参数，系统账户，通用冻结，已冻结金额为1200，余额2000，冻结600
     * 1014 全填参数，系统账户，充值冻结，已冻结金额为1300，余额1500，冻结100
     * 1015 全填参数，系统账户，提现冻结，已冻结金额为1400，余额3000，冻结1000
     * 1016 全填参数，保证金账户，通用冻结，已冻结金额为1500，余额3000，冻结700
     * 1017 全填参数，保证金账户，充值冻结，已冻结金额为1600，余额3000，冻结1000
     * 1018 全填参数，保证金账户，提现冻结，已冻结金额为1700，余额3000，冻结100
     * 1019 重复冻结，只冻结一次金额
     * 1020 冻结账户存在联动账户，联动冻结，冻结100，联动账户
     * 1021 冻结账户userId+tag存在联动账户userId+tag，联动冻结，冻结100，联动账户
     * 1023 userId+tag 标准账户，充值冻结，已冻结金额为100，余额1100，冻结1000
     */
    @AutoTest(file = "user/accountTransServiceFreezeBalanceTestSuccess.csv")
    @DisplayName("冻结金额--成功用例")
    public void accountTransServiceFreezeBalanceTestSuccess(
            // 基本参数
            int testId,
            // 业务参数
        ...
        if (testId == 1019) {
            // 重复调用接口
            accountTransService.freezeBalance(accountFreezeOrder);
        }
        // 调用接口
        BizResult<AccountTradeResultInfo> result = accountTransService.freezeBalance(accountFreezeOrder);
        // 结果验证
        ...
        // 数据库验证
        // account表数据
        AccountDO DO = userTestBase.findAccountByAccountNo(accountNo).get(0);
        ...
        if (testId >= 1020) {
            assertEquals(accountDO2.getFreezeBalance() + accountFreezeOrder.getAmount().getCent(), accDO2.getFreezeBalance());
        ...
        // account_freeze_log表数据
        AccountFreezeLogDO DO1 = userTestBase.findAccountFreezeLogByAccountNo(accountNo).get(0);

        assertEquals(accountFreezeOrder.getAmount().getCent(), DO1.getAmount());
        assertEquals(accountNo, DO1.getAccountNo());
        ...
        assertNotNull(DO1.getFreezeCode());
        assertEquals(FreezeType.FREEZE.toString(), DO1.getFreezeType());
        ...
        // account_freeze_code_amount表数据
        AccountFreezeCodeAmountDO DO2 = userTestBase.findAccountFreezeCodeAmountByAccountNo(accountNo).get(0);

        assertEquals(accountNo, DO2.getAccountNo());
        assertEquals(accountFreezeOrder.getFreezeCode().code(), DO2.getFreezeCode());
        ...
        assertEquals(new Long(0), DO2.getUnfreezeAmount());
        if (testId >= 1020) {
            ...
            assertEquals(accountFreezeOrder.getFreezeCode().code(), DO3.getFreezeCode());
        }
     ...
     */
    @AutoTest(file = "user/accountTransServiceFreezeBalanceTestFailOne.csv")
    @DisplayName("冻结金额--失败用例一")
    public void accountTransServiceFreezeBalanceTestFailOne(
            // 基本参数
            int testId,
            // 业务参数
            AccountFreezeOrder accountFreezeOrder,
     ...
     * 1005 关联账户状态为冻结状态
     * 1006 关联账户余额不足
     * 1007 主账户userId+tag不存在
     * 1008 主账户userId+tag存在,关联账户userId+tag不存在
     * 1009 主账户userId+tag存在,关联账户userId+tag不存在
     */
    @AutoTest(file = "user/accountTransServiceFreezeBalanceTestFailTwo.csv")
    @DisplayName("冻结金额--失败用例二")
    public void accountTransServiceFreezeBalanceTestFailTwo(
            // 基本参数
            int testId,
            // 业务参数
            AccountFreezeOrder accountFreezeOrder,
